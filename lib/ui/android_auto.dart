...
class AndroidAuto {
  ...
  Future<void> playItem(String id) async {
    ...
    if (id == '${prefix}tracks') {
      ...
      try {
        favPlaylist = await deezerAPI.fullPlaylist(
    ...
    if (id.startsWith('${prefix}playlist')) {
      Playlist p = await deezerAPI.fullPlaylist(
        id.replaceFirst('${prefix}playlist', ''),
      );
      await GetIt.I<AudioPlayerHandler>().playFromPlaylist(
        p,
        p.tracks?[0].id ?? '',
      );
      return;
    ...
    if (id.startsWith('${prefix}album')) {
      ...
      await GetIt.I<AudioPlayerHandler>().playFromAlbum(
        a,
        a.tracks?[0].id ?? '',
      );
      return;
    ...
    if (id.startsWith('${prefix}stl')) {
      SmartTrackList stl = await deezerAPI.smartTrackList(
        id.replaceFirst('${prefix}stl', ''),
      );
